...
for subjix = 1 : nsubj,subjix % Loop through subjects
  ...
  for hemi = 1 : 2,hemi % For both hemis
      ...
      for i = 1 : num_roi,i % Loop through ROIs
      
          ix=find((a1.img>i-1) & (a1.img<i+1)); % Find required voxels
...
req = meanse(:,:,:,:,order,:);
save('NSDSynth_8subj2hemi5roi2tasks72groupsmnandse_b2.mat','req');
for subjix = 1:nsubj
 for a = 1:2    
    for i = 1:5
        ...
        drop0 = '/home/stone/generic/Dropbox/nsdanalysis/asha/NSD_Synth/Beta2';
        nam = sprintf('%s/Image_Group_Plots/Subject_%02d/%ch_Hemisphere/%s/',drop0,subjix,hemis(a),roi{i});
        filename0 = sprintf('NSDSyn_beta2_%02d%02d%02d_Group',subjix,a,i);
...
for subjix = 1: nsubj 
    for hem = 1:2
        for i = 1:5
            for tsk = 1:2
             if(~(sum(abs(req(subjix,hem,i,:,:,1)))>0)) % if ROI doesn't exists   
             
                 subjix,hem,i
              grpavg1(subjix,hem,i,tsk,:) = NaN;  
             else
                 grpavg1(subjix,hem,i,tsk,:) = reshape(req(subjix,hem,i,tsk,:,1),1,num_group + 1);
             end
             end
        end
    end
...
for subjix = 1 : nsubj
        for i = 1:5
            scale_val = norm(reshape(hemavg(subjix,1,i,:,:),1,72*2)); % One norm for both tasks 
            hemavg(subjix,1,i,:,:) = hemavg(subjix,1,i,:,:)./scale_val;
        end
end
            

grpmean = nanmean(hemavg,1);
grpstd = nanstd(hemavg,0,1);
...
save('NSDSynth_Groupavg_5roi_2tasks_72groups_mnandse_b2.mat','gmean','gse'); 
...
for i = 1:5
        ...
        subplot(5,1,count)
        errorbar3(1:num_group+1,reshape(s_val(i).*gmean(i,1,:),1,num_group+1),reshape(grpse2(i,1,:),1,num_group+1),1,[1 0.5 0.5])
        hold on
        errorbar3(1:num_group+1,reshape(grpmean2(i,2,:),1,num_group+1),reshape(grpse2(i,2,:),1,num_group+1),1,[0.5 0.5 1])
        plot(1:num_group+1,reshape(grpmean2(i,1,:),1,num_group+1),'r','LineWidth',2)
        plot(1:num_group+1,reshape(grpmean2(i,2,:),1,num_group+1),'b','LineWidth',2)
        straightline([3.5,7.5,12.5,17.5,22.5,27.5,33.5,40.5,46.5,52.5,56.5,72.5],'v','--k');
        straightline(0,'h','-k');
        h1 = straightline(mean(grpmean2(i,1,:),3),'h','m');
        h2 = straightline(mean(grpmean2(i,2,:),3),'h','c');
        set(h1,'LineWidth',2);
        set(h2,'LineWidth',2);
        xlim([0 73])
        ...
        filename0 = sprintf('NSDSyn_Group_Average_b2');
