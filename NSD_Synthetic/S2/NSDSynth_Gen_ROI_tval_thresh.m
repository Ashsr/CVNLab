% For floc thresholding

clear all
ROI_name = input('Enter ROI name ','s');
thresh_val = input('Enter the t-val threshold value');

B2_MeanPSC2(ROI_name, thresh_val)
...
for subjix = 1 : nsubj,subjix
    for hemi = 1 : 2,hemi
        ...
        if ROI_name == 'floc-faces'    
             a2 = load_untouch_nii(sprintf('%s/ppdata/subj%02d/func1mm/floc_facestval.nii.gz',dir_roi,subjix));
        else
            a2 = load_untouch_nii(sprintf('%s/ppdata/subj%02d/func1mm/floc_wordtval.nii.gz',dir_roi,subjix));
        end
        for i = 1 : num_roi,i
            
            ix=find((a1.img==i) & (a2.img>thresh_val)); % Find required voxels by limiting the threshold
...
save(sprintf('%s/NSDSynth_8subj2hemi%droi2tasks72groupsmnandse_b3_%s_tval_thresh_%d.mat',drop0,num_roi,ROI_name,thresh_val),'req');
...
for subjix = 1:nsubj
  for hemi = 1:2    
    for i = 1:num_roi
        ...
        filename0 = sprintf('NSDSyn_%02d%02d%02d_Group_%dtval',subjix,hemi,i,thresh_val);
...
save(sprintf('%s/NSDSynth_Groupavg_%droi_2tasks_72groups_mnandse_b3_%s_%dtval.mat',drop0,num_roi,ROI_name,thresh_val),'gmean','gse'); 
load (sprintf('NSDSynth_Grandmean_%dROI_Beta_ver2_%s_%dtval.mat',num_roi,ROI_name,thresh_val)) % change for every ROI
...
save(sprintf('%s/NSDSynth_Groupavg_%droi_2tasks_72groups_mnandse_b3_ver2_%s_%dtval.mat',drop0,num_roi,ROI_name,thresh_val),'gmean2','gse2'); 
...
for i = 1:num_roi
        ...
        filename0 = sprintf('NSDSyn_Group_Average_b3_fixed_%dtval',thresh_val);
