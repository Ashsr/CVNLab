...
for subjix = 1:nsubj
  for hemi = 1:2    
    for i = 1:5
        ...
        straightline(0,'h','-k')
        title(sprintf('Subj %02d - %ch : ROI %s',subjix,hemis(hemi),roi{i}));

        drop0 = '/home/stone/generic/Dropbox/nsdanalysis/asha/NSD_Synth/Beta_3';
        nam = sprintf('%s/Image_Group_Plots/Subject_%02d/%ch_Hemisphere/%s/',drop0,subjix,hemis(hemi),roi{i});
...
tot_picks = 72;
for i = 1:tot_itr
samp1 = ceil(rand(1,tot_picks)*(num_group+1));
samp2 = ceil(rand(1,tot_picks)*(num_group+1));
mean_ratio(i,:)= reshape(mean(gmean2(:,2,samp1),3)./mean(gmean2(:,1,samp2),3),1,num_roi); % mean from task 1/mean from task 2 
end
...
for i = 1:num_roi
    subplot(5,1,i)
    hist(mean_ratio(:,i),1000)
    title(sprintf('%s',roi{i}))
end
drop0 = '/home/stone/generic/Dropbox/nsdanalysis/asha/NSD_Synth/Beta_3';
nam = sprintf('%s/Mean_ratio/',drop0);
filename0 = sprintf('NSDSyn_ROI_Mean_ratio_hist');
figurewrite(filename0,f2,[],nam);

bigmean_ratio = mean(gmean2(:,2,:),3)./mean(gmean2(:,1,:),3);
bigmode_ratio = mode(gmean2(:,2,:),3)./mode(gmean2(:,1,:),3);
samplemean_ratio_mean = mean(mean_ratio,2);
samplemean_ratio_mode = mode(mean_ratio,2);
samplemean_ratio_se = std(mean_ratio,0,2)./sqrt(tot_itr);
 

f3 = figureprep([0 0 3500 2500]);
for i = 1:num_roi
    subplot(5,1,i)
    hist(mean_ratio(i,:),10000)
    xlim([-5 5])
    ylim([0 20])
    hold on
    scatter(bigmean_ratio(i),10,'db','filled')
    scatter(bigmode_ratio(i),10,'dr','filled')
    %scatter(samplemean_ratio_mode(i),10,'go','filled')
    errorbar(samplemean_ratio_mean(i),10,samplemean_ratio_se(i),'horizontal','Marker','o')
    title(sprintf('%s',roi{i}))
end
%drop0 = '/home/stone/generic/Dropbox/nsdanalysis/asha/NSD_Synth/Beta_3';
%nam = sprintf('%s/Mean_ratio/',drop0);
filename0 = sprintf('NSDSyn_ROI_Mean_ratio_hist_fixedaxis');
figurewrite(filename0,f3,[],nam);

f4 = figureprep([0 0 3500 2500]);
plot(bigmean_ratio,'b*','MarkerSize',16)
hold on
plot(bigmode_ratio,'g+','MarkerSize',16)
%plot(samplemean_ratio_mode,'kX','MarkerSize',16)
errorbar(samplemean_ratio_mean,samplemean_ratio_se,'ro','MarkerSize',16)
xlim([0 6])
xticklabels({'0','IOG','pFUS','mFUS','mTL','aTL'})
filename0 = sprintf('NSDSyn_ROI_Mean_ratio_Meanandse');
figurewrite(filename0,f4,[],nam);
