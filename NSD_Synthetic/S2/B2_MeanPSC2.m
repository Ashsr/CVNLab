...
function B2_MeanPSC2(ROI_name,tval)
%Define variables
nsubj = 8;
...
for subjix = 1 : nsubj,subjix % Loop over Subjects
  ...
  for hemi = 1 : 2,hemi % For both hemis
      ...
      for i = 1 : num_roi,i % Loop over ROIs
          ...
          ix=find((a1.img == i) & (a2.img>tval)); % Find required voxels by limiting the threshold
...
save(sprintf('NSDSynth_8subj_%dROI_MeanPSC_Beta_ver2_%s_%dtval.mat',num_roi,ROI_name,tval),'mn2'); % Save out hemi averaged means (subj x ROI)
save(sprintf('NSDSynth_Grandmean_%dROI_Beta_ver2_%s_%dtval.mat',num_roi,ROI_name,tval),'mn3','se3'); % Save out group average and se
end
