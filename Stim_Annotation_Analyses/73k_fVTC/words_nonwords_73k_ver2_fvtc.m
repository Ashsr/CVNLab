% T-test and angle metric for words - non words in 73k stimulus images for
% the whole of fVTC - beta 2

clear all
filename = '/home/surly-raid4/kendrick-data/nsd/nsdextensions/NSD_Annotation_Efforts_1.0/Automated/Words/binaryimage_mode2.mat';
...
for subjix = 1:nsubj, subjix % Loop over subjects
    ...
    for hem = 1:length(hemis), hem % Loop over hemis
        ...
        wix_data = []; % To store the averaged values for images with words
        
        nwix_data = []; % To store the averaged values for images without words
        ...
        chk = ismember(rept(subjix,1,1:totimg(subjix)),wix);
        wix_data = tr_avg(:,chk);
        nwix_data = tr_avg(:,~chk);
        ...
        se1 = std(wix_data,[],2)/sqrt(size(wix_data,2));
        
        mn2 = mean(nwix_data,2);
        se2 = std(nwix_data,[],2)/sqrt(size(nwix_data,2));
    ...
    imwrite(rgbimg,sprintf('Subject%02d/word_nonword_3_tvals_10k_subj%02d.png',subjix,subjix));
    Lookup = [];
    cvnlookup(sprintf('subj%02d',subjix),3,hem_angmet,[-90,90], cmapsign4(256));
    imwrite(rgbimg,sprintf('Subject%02d/word_nonword_3_angvals_10k_subj%02d.png',subjix,subjix));
    
    close all
end
save('words_nonwords_73k_fVTC_full.mat','data','wix','nwix','am_tvals','-v7.3')
