% Split half tvals and angmet for fVTC all 10k images
load repmat.mat
load faces_nonfaces_73k_fVTC_full
...
for subjix = 1:nsubj, subjix
    ...
    for hem = 1:2, hem
        ...
        sp1_data = tr_avg(:,sp1);
        sp2_data = tr_avg(:,sp2);
        chk = ismember(rept(subjix,1,sp1),fix);
        sp1_fix_data = sp1_data(:,chk);
        sp1_nfix_data = sp1_data(:,~chk);
        chk2 = ismember(rept(subjix,1,sp2),fix);
    ...
    imwrite(rgbimg,sprintf('sp1/Subject%02d/face_nonface_3_tvals_10k_subj%02d_sp1.png',subjix,subjix));
    Lookup = [];
    cvnlookup(sprintf('subj%02d',subjix),3,hem_tval_sp2,[-10,10],cmapsign4(256));
    imwrite(rgbimg,sprintf('sp2/Subject%02d/face_nonface_3_tvals_10k_subj%02d_sp2.png',subjix,subjix));
    Lookup = [];
    cvnlookup(sprintf('subj%02d',subjix),3,hem_angmet_sp1,[-90,90],cmapsign4(256));
    imwrite(rgbimg,sprintf('sp1/Subject%02d/face_nonface_3_angvals_10k_subj%02d_sp1.png',subjix,subjix));
    Lookup = [];
    cvnlookup(sprintf('subj%02d',subjix),3,hem_angmet_sp2,[-90,90],cmapsign4(256));
    imwrite(rgbimg,sprintf('sp2/Subject%02d/face_nonface_3_angvals_10k_subj%02d_sp2.png',subjix,subjix));
    
    close all
end
save('faces_nonfaces_73k_fVTC_sp1_sp2.mat','am_tvals_sp','-v7.3')
