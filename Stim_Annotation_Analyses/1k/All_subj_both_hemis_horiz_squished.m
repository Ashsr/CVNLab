...
for i = 1:nsubj
    ...
    plot(nanmean(rh_t,2))
    hold on
    plot(1:size(nanmean(rh_t,2)),mean(nanmean(rh_t,2)),'-k')
    hold off
    %ylim([-10 10])
    title( sprintf('Subj%02d : RH - \n Horizontally Squished',i))
   
    subplot(8,8,count+2)
    imagesc(rh_t_img(1:10:end,:,:))
    title(sprintf('Subj%02d : RH - Box crop \n (skipping 10 pixels)',i))
    subplot(8,8,count + 3)
    plot(nanmean(rh_t(1:10:end,:),2))
    hold on
    plot(1:0.1:size(nanmean(rh_t(1:10:end,:),2)),mean(nanmean(rh_t(1:10:end,:),2)),'-k')
    hold off
    %ylim([-10 10])
    ...
    plot(nanmean(lh_t,2))
    hold on
    plot(1:size(nanmean(lh_t,2)),mean(nanmean(lh_t,2)),'-k')
    hold off
    %ylim([-10 10])
    ...
    subplot(8,8,count + 7)
    plot(nanmean(lh_t(1:10:end,:),2))
        hold on
    plot(1:0.1:size(nanmean(lh_t(1:10:end,:),2)),mean(nanmean(lh_t(1:10:end,:),2)),'-k')
    hold off
    %ylim([-10 10])
    title( sprintf('Subj%02d : LH - \n Horizontally Squished \n (skipping every 10 pixels)',i))
    count = count + nsubj;
