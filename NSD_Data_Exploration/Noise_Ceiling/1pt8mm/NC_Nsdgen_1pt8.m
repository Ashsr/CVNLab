...
% Loading and zscoring 
  ...
  for subjix = 1:nsubj
      ...
      a1 = load_untouch_nii(gunziptemp(sprintf('%s/ppdata/subj%02d/func1pt8mm/roi/nsdgeneral.nii.gz',nsddatadir,subjix))); % Load mask
      ...
      % Depending on the beta preparation chosen the corresponding file is accessed to get the betas 
        if betatype == 1
          a3 = matfile([nsddatadir '_betas/ppdata/subj' sprintf('%02d',subjix) '/func1pt8mm/betas_assumehrf/betas_session' sprintf('%02d',sess) '.mat']); % Get betas from the selected voxels from each session
        else
          if betatype == 2 
            a3 = matfile([nsddatadir '_betas/ppdata/subj' sprintf('%02d',subjix) '/func1pt8mm/betas_fithrf/betas_session' sprintf('%02d',sess) '.mat']); % Get betas from the selected voxels from each session
          else 
            if betatype == 3
              a3 = matfile([nsddatadir '_betas/ppdata/subj' sprintf('%02d',subjix) '/func1pt8mm/betas_fithrf_GLMdenoise_RR/betas_session' sprintf('%02d',sess) '.mat']); % Get betas from the selected voxels from each session
            else
        ...
        %find(isnan(zsc))
        %zsc(isnan(zsc)) = 0; % Eliminate NANs, sess 4 subj 6 has NAN
        %values for nsess = 15
  ...
  for p=1:length(noiselevels)
    for rep=1:5 % To make the simulation more smoother 
      sig = 0 + s*randn(1,t1);  % for 10,000 images, the true mean
      noise = noiselevels(p) * randn(1,ntps*nsess);  % the noise to add to each of 3 trials for each distinct image
      ...
      for i = 1: ntps*nsess
         ...
         while found == 0 
          if i == rept(1,2,count)
              data(i) = sig(j) + noise(i);
              j = j+1; % Think here
              count = count + 1;
              found = 1;
          else
              if i > rept(1,2,count) % almost never happens - but still while condition on top
                  count = count + 1;
              else
                  if i < rept(1,2,count)
                      count2 = 1;
                      while count2 <=t1 && found == 0
                          if i == rept(1,3,count2)
                              data(i) = sig(rept(1,2,count2)) + noise(i);
                      ...
                      if found == 0
                          while count3 <= t1
                              if i == rept(1,4,count3)
                                  data(i) = sig(rept(1,2,count3)) + noise(i);
      ...
      for r = 1:k2
          for trialn = 1:2
              tval = a_two(r,trialn); % Find trial number
      ...
      for r = 1:k3
          for trialn = 1:3
              tval = a_three(r,trialn); % Find trial number
  ...
  for subjix=1:nsubj
    ...
    if betatype == 1
     nam3 = [nsddatadir '_betas/ppdata/subj' sprintf('%02d',subjix) '/func1pt8mm/betas_assumehrf/noiseceiling_1trial.nii.gz'];
     nam4 = [nsddatadir '_betas/ppdata/subj' sprintf('%02d',subjix) '/func1pt8mm/betas_assumehrf/noiseceiling_2trial.nii.gz'];
     nam5 = [nsddatadir '_betas/ppdata/subj' sprintf('%02d',subjix) '/func1pt8mm/betas_assumehrf/noiseceiling_3trial.nii.gz'];
    else
        if betatype == 2
            nam3 = [nsddatadir '_betas/ppdata/subj' sprintf('%02d',subjix) '/func1pt8mm/betas_fithrf/noiseceiling_1trial.nii.gz'];
            nam4 = [nsddatadir '_betas/ppdata/subj' sprintf('%02d',subjix) '/func1pt8mm/betas_fithrf/noiseceiling_2trial.nii.gz'];
            nam5 = [nsddatadir '_betas/ppdata/subj' sprintf('%02d',subjix) '/func1pt8mm/betas_fithrf/noiseceiling_3trial.nii.gz'];
        else
            nam3 = [nsddatadir '_betas/ppdata/subj' sprintf('%02d',subjix) '/func1pt8mm/betas_fithrf_GLMdenoise_RR/noiseceiling_1trial.nii.gz'];
            nam4 = [nsddatadir '_betas/ppdata/subj' sprintf('%02d',subjix) '/func1pt8mm/betas_fithrf_GLMdenoise_RR/noiseceiling_2trial.nii.gz'];
            nam5 = [nsddatadir '_betas/ppdata/subj' sprintf('%02d',subjix) '/func1pt8mm/betas_fithrf_GLMdenoise_RR/noiseceiling_3trial.nii.gz'];
        end
    end
            
     
    nsd_savenifti(vol1,[1.8 1.8 1.8],nam3,4/3);
    nsd_savenifti(vol2,[1.8 1.8 1.8],nam4,4/3);
    nsd_savenifti(vol3,[1.8 1.8 1.8],nam5,4/3);
    ...
    for level = 1:3
      ...
      % flatmaps
      data1 =  nsd_mapdata(subjix,'func1pt8',sprintf('lh.layerB%01d',level),vol3,'nearest');
      data2 = nsd_mapdata(subjix,'func1pt8',sprintf('rh.layerB%01d',level),vol3,'nearest');
