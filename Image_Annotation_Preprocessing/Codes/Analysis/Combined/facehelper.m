...
function output = facehelper(fname,label_vals,mode_val)
...
facecountmat = zeros(totimg,9); % initialise count matrix
binaryimagemat = zeros(425,425,totimg,9); % initialise binary image matrix
...
% Calculate the values and store and output according to user input
 ...
 switch mode_val
     case 1
          output = sum(facecountmat(:,[label_vals]),2); % mode 1 sum the count matrix according to the input label
